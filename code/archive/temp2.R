...
plot_composition <- function(proteome, target_phi,
  ...
  if(legend){
    ...
    legend(-0.03*max(target_phi), 1.53, legend = leg_text, fill = rev(colors), bty = "n", 
...
for(modelname in models_to_plot){
  ...
  n_rxns <- 2 #length(unique(data$reaction))
  
  suffix <- ifelse(relative_phi, "_rel", "_abs")
  png(here("figures", paste0(modelname, suffix, "_presi.png")),
      type="cairo", units="cm",
      width=20.5, height=4*n_rxns, res=300)
  par(mfrow=c(n_rxns, 3), mar = c(0.8,4.5,0.8,0.5), oma = c(3.3,1,3,0))
  
  for(rxn in unique(data$reaction)){
    if(!rxn %in% c("AAS", "NTS")){next}
    ...
    # if(rxn == "NTS"){
      mtext(bquote("Growth rate relative to optimum " * mu / mu^"\u204E"), 
            side = 2, cex = cex_lab, line = axis_label_line, at = 1.2)
    }
    ...
    plot_legend <- ifelse(rxn == "AAS", TRUE, FALSE)
    ...
    # }
    # if(rxn == "NTS"){
      mtext("Proteome mass fraction", side = 2, cex = cex_lab, line = axis_label_line, at = 1.2)
    }
    ...
    # }
    # if(rxn == "NTS"){
      mtext("Biomass fraction", side = 2, cex = cex_lab, line = axis_label_line, at = 1.2)
    }
    ...
    mtext(xlabel, side = 1,
          outer = TRUE, cex = cex_lab, line = 1.6, adj = 0.52)
  }
  dev.off()
}
