...
process_reaction <- function(reaction_name, opt_phi, q0_initial) {
  ...
  if(opt_phi/stepsize < 200){
    below <- seq(round(opt_phi, 4), 0, length.out = 300)

  }
  above <- seq(round(opt_phi, 4), 0.8, by = stepsize)
...
write.csv(results, 
          here("data", paste0(modelname, "_prot_cost_kf_", kcat_factor, ".csv")), 
          row.names = FALSE)
