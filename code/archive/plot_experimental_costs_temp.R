...
for(exp in unique(all_data$experiment)){
  ...
  for(cond in seq_along(conditions)){
    ...
    labels <- if (exp %in% c(3,4)) c(0:5) else FALSE
    axis(1, at = 0:5, labels = labels)
    axis(2, las = 2)# at = c(0, 0.25, 0.5, 0.75, 1), labels = labels)
    box()
  }
