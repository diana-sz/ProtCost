...
opt_data <- read.csv(paste0("../data/", modelname, "_dekel.csv"), row.names = 1)
...
plot_composition <- function(proteome, target_phi, colors = NULL,
  ...
  if(legend){
    ...
    legend(xlim[1], -0.33,  #"bottomleft", 
           legend = leg_text, fill = rev(colors), bty = "n", cex = 1, ncol = 4)
    par(xpd = FALSE)
...
plot(mu ~ phi, data = opt_data, 
     ...
     axes = FALSE,
     ylim = c(0, max(mu)),
     col = point_cols, pch = point_shapes)
box()
...
plot(NA,
     ...
     xlim = xlim,
     axes = FALSE,
     ylim = c(0.3, 1.2)) #max(opt_data$mu_ref)))
box()
...
for(pphi in seq_along(plotted_phis)){
  ...
  points(mu_ref ~ x_C2, data = one_phi, col = point_cols, pch = point_shapes, cex = 1.2)
  text(max(x_C2_vals)-8, (one_phi[which.max(one_phi$x_C2), "mu_ref"]-0.035), 
       bquote(Phi * " = " * .(plotted_phis[pphi])), cex = 1, adj=0)
}


points(dekel_conc, dekel_rel_mu, col = "grey50", pch = 24)
...
mtext("LAC concentration", side = 1, line = 2.8, cex = cex_lab)
...
for (p in seq_along(plotted_phis)) {
  ...
  mtext(bquote(Phi * " = " * .(phi)), side=3, cex = cex_lab*0.9, line = 0.3)
  ...
  legend(xlim[1], ylim[1], #inset = c(-0.05,-0.05), 
         legend = gsub("_", " ", plots), col = colors, cex = 1, lwd = 2,
         lty = 1:length(plots), bty = "n", horiz=FALSE, ncol=2)
  par(xpd = FALSE)
  }
  ...
  colors2 <- brewer.pal(ncol(biomass), "RdBu")
