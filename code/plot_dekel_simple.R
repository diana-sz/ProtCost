...
png(here("figures", paste0(modelname, "_main.png")), 
    type="cairo", units="cm",
...
for(pphi in seq_along(plotted_phis)){
  ...
  text(min(x_C2_vals), (one_phi[which.min(one_phi$x_C2), "mu_ref"]+0.055), 
       bquote(italic("\u03A6")["LAC"] * " = " * .(plotted_phis[pphi])), cex = 0.9, adj=0)
}
...
for (p in seq_along(plotted_phis)) {    
  ...
  plot_composition(biomass, concs, colors2, main="", 
                   ...
                   xaxis = draw_axis,
                   xlim = xlim)
  mtext(bquote(italic("\u03A6")["LAC"] * " = " * .(phi)), side=3, cex = 0.65, line = 0.15, adj = 1)
